...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemClickListener, BluetoothConnection.OnReadNewLineListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvChatOutput = (TextView) findViewById(R.id.tvChatOutput);
            ...
            case REQUEST_ENABLE_BLUETOOTH:
                switch (resultCode) {
                    case RESULT_OK:
                        ...
                        break;
                    default:
                        new AlertDialog.Builder(this).setTitle("提醒").setMessage("您拒绝启动蓝牙").setPositiveButton("再次启用", new DialogInterface.OnClickListener() {
                            @Override
    ...
    public void btnSendLineClicked(View view) {
        if (!TextUtils.isEmpty(etChatInput.getText())) {
            connection.sendLine(etChatInput.getText().toString());
        }
    }
    ...
    public void onRead(final String line, final BluetoothDevice remoteDevice) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                tvChatOutput.append(remoteDevice.getName() + ":" + line + "\n");
            }
        });
    }
}
